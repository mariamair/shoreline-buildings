...
export const connectToDatabase = async (connectionString) => {
  ...
  connection.on('connected', () => console.info('Mongoose connected to MongoDB.'))
  connection.on('error', (err) => console.error(`Mongoose connection error: ${err}`))
  connection.on('disconnected', () => console.info('Mongoose disconnected from MongoDB.'))
  ...
  for (const signalEvent of ['SIGINT', 'SIGTERM']) {
    process.on(signalEvent, () => {
      (async () => {
        ...
        console.info(`Mongoose disconnected from MongoDB through ${signalEvent}.`)
        process.exit(0)
      })()
    })
  }
  ...
  console.info('Mongoose connecting to MongoDB.')
